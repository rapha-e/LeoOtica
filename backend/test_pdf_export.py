...
def test_pdf_export():
    url = "http://localhost:8000/api/v1/alerts/export-pdf?lead_time_days=7&safety_days=5&coverage_days=15"
    print("Enviando requisição GET para exportar relatório PDF...")
    import json
    import pytest
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            assert "application/pdf" in content_type, "Erro: Content-Type incorreto (deve ser application/pdf)"
            
            # Salva o arquivo localmente para inspeção se necessário
            output_path = os.path.join(os.path.dirname(__file__), "teste_pedido.pdf")
            with open(output_path, "wb") as f:
                f.write(response.read())
            print(f"PDF exportado com sucesso e salvo em: {output_path}")
