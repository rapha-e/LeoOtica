          ...
          <div className="modal-content" style={{ maxWidth: '800px', width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: '40px' }}>
            ...
            {loadingInvoice ? (
              ...
              <div id="invoice-print-area">
                ...
                {/* Área de Quitação / Instruções (oculta na impressão se for PDF) */}
                <div className="no-print" style={{ marginTop: '30px', padding: '20px', borderRadius: '12px', border: '1px dashed hsl(var(--primary) / 0.3)', background: 'hsl(var(--primary) / 0.02)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h5 style={{ margin: '0 0 4px 0', fontSize: '0.9rem' }}>Instruções de Pagamento</h5>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: 'hsl(var(--text-secondary))' }}>
                      Realize o depósito/PIX para a conta cadastrada do laboratório e clique em <strong>Quitar Cobrança</strong> para liquidar a pendência.
                    </p>
                  </div>
                  ...
                  {invoiceDetail.status === 'FECHADO' && (
                    <button 
                      className="btn btn-primary"
                      onClick={() => handlePayCycle(invoiceDetail.id)}
                      style={{ background: 'hsl(var(--success))', boxShadow: 'none' }}
                    >
                      <Check size={16} /> Liquidar Cobrança
                    </button>
                  )}
                </div>

                {/* Painel de Ações Fiscais (oculto na impressão) */}
                <div className="no-print" style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(147, 51, 234, 0.1)', paddingBottom: '10px' }}>
                    <h5 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700, color: 'hsl(var(--primary))', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <Landmark size={18} /> Painel Fiscal de Saída (NF-e)
                    </h5>
                    {invoiceDetail.nfe_saida ? (
                      <span style={{ 
                        ...
                        padding: '4px 10px', 
                        borderRadius: '20px', 
                        background: invoiceDetail.nfe_saida.status === 'EMITIDA' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                        color: invoiceDetail.nfe_saida.status === 'EMITIDA' ? 'rgb(16, 185, 129)' : 'rgb(239, 68, 68)'
                      }}>
                        {invoiceDetail.nfe_saida.status === 'EMITIDA' ? 'NF-e Autorizada' : 'NF-e Cancelada'}
                      </span>
                    ) : (
                      <span style={{ fontSize: '0.75rem', color: 'hsl(var(--text-muted))', fontWeight: 600 }}>
                        Nota fiscal pendente de emissão
                      </span>
                    )}
                  </div>
                  ...
                  {!invoiceDetail.nfe_saida ? (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
                      <p style={{ margin: 0, fontSize: '0.8rem', color: 'hsl(var(--text-secondary))', maxWidth: '450px' }}>
                        Gere a Nota Fiscal Eletrônica (NF-e) simplificada para o faturamento em lote das OSs autorizadas. A nota conterá os valores e dados do cliente ótica.
                      </p>
                      <button 
                        className="btn btn-primary"
                      ...
                      >
                        {emittingNfe ? <RefreshCw size={16} className="spin" /> : <Check size={16} />}
                        Emitir NF-e
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', background: 'white', padding: '12px', borderRadius: '8px', border: '1px solid rgba(224, 230, 240, 0.5)' }}>
                        <div>
                          <span style={{ fontSize: '0.7rem', color: 'hsl(var(--text-muted))', display: 'block' }}>NÚMERO DA NOTA</span>
                          <strong style={{ fontSize: '0.85rem' }}>Nº {String(invoiceDetail.nfe_saida.nfe_number).padStart(6, '0')} (Série {invoiceDetail.nfe_saida.serie})</strong>
                        </div>
                        <div>
                          <span style={{ fontSize: '0.7rem', color: 'hsl(var(--text-muted))', display: 'block' }}>DATA DE EMISSÃO</span>
                          <span style={{ fontSize: '0.85rem' }}>{formatDateTime(invoiceDetail.nfe_saida.emitted_at)}</span>
                        </div>
                        <div style={{ gridColumn: 'span 2' }}>
                          <span style={{ fontSize: '0.7rem', color: 'hsl(var(--text-muted))', display: 'block' }}>CHAVE DE ACESSO</span>
                          <span style={{ fontSize: '0.8rem', fontFamily: 'monospace', wordBreak: 'break-all', fontWeight: 600 }}>
                            {invoiceDetail.nfe_saida.chave_acesso.replace(/(.{4})/g, '$1 ')}
                          </span>
                        </div>
                      </div>

                      {/* Ações da Nota Fiscal */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginTop: '5px' }}>
                        <div style={{ display: 'flex', gap: '10px' }}>
                          ...
                          >
                            {downloadingDanfe ? <RefreshCw size={14} className="spin" /> : <FileText size={14} />}
                            Visualizar DANFE
                          </button>
                          ...
                          <button 
                            ...
                            style={{ height: '36px', padding: '0 12px', fontSize: '0.8rem' }}
                            title="Baixar arquivo XML regulatório da NF-e"
                          >
                            {downloadingXml ? <RefreshCw size={14} className="spin" /> : <Eye size={14} />}
                            Baixar XML
                          </button>
                        </div>

                        {invoiceDetail.nfe_saida.status === 'EMITIDA' && (
                          <button 
                          ...
                          >
                            {cancellingNfe ? <RefreshCw size={14} className="spin" /> : <X size={14} />}
                            Cancelar NF-e
                          </button>
                        )}
                      </div>
                ...
                <div className="no-print" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '30px', borderTop: '1px solid rgba(224,230,240,0.8)', paddingTop: '20px' }}>
                  <button 
                    className="btn btn-secondary" 
                    onClick={() => setIsInvoiceModalOpen(false)}
                  >
                  ...
                  >
                    {exportingPdf ? <RefreshCw size={16} className="spin" /> : <Eye size={16} />}
                    Exportar PDF
                  </button>
                  ...
                  >
                    {exportingExcel ? <RefreshCw size={16} className="spin" /> : <FileText size={16} />}
                    Exportar Excel
                  </button>
